...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        exclude = ['is_superuser',
                   ...
                   'password',
                   'date_joined']
        read_only_fields = ('phone','role','last_activity','last_login') 
...
class UserSerializer(WritableNestedModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('last_login','last_activity','date_joined','role')
...
class GetUserByCardNumberSerializer(serializers.Serializer):
    barcode = serializers.CharField(required=True)

    def create(self, validated_data):
        barcode = validated_data.pop('barcode')
        if barcode:
            user = get_object_or_none(User, barcode=barcode)
        
        if user:
            return user
        
        raise serializers.ValidationError({"barcode": ["Такого пользователья не существует"]})



class SetPinCodeSerializer(serializers.Serializer):
    pin_code = serializers.CharField(max_length=6)



class VerifyPinCodeSerializer(serializers.Serializer):
    pin_code = serializers.CharField(max_length=6)
