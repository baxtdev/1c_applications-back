...
def generate_barcode(third_letter):
    if len(third_letter) != 1 or not third_letter.isalpha():
        raise ValueError("Третья буква должна быть одной буквой.")

    digits = ''.join([str(random.randint(0, 9)) for _ in range(6)])

    code = f"IN{third_letter}{digits}"
    return code
...
def to_chek_recipte(url):
    headers = {"Content-Type": "application/xml"}
    response = requests.get(url,headers=headers)
    chek_data = response.json()
    data = {"check_code":chek_data['fnSerialNumber'] ,"chek_amount":chek_data['ticketTotalSum'],'gas_station':f"{chek_data['crData']['locationName']}-{chek_data['crData']['locationAddress']}"}
    return data   
